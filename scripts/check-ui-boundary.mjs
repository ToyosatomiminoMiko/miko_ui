 ...
 * | exports-surface | `exports` 只指向 `index.ts` 与 `styles/`,内部路径不进公开面 |
 ...
 *
 * 用法:
 *   node scripts/check-ui-boundary.mjs            # 与 baseline 比对;出现**新增**违例就非零退出
 *   node scripts/check-ui-boundary.mjs --update   # 按当前结果重写 baseline(耦合变少时用)
 *   node scripts/check-ui-boundary.mjs --list     # 打印全部违例明细
 *
...
function checkExports() {
    ...
    const visit = (value, key) => {
        if (typeof value === 'string') {
            const ok = value === './src/index.ts' || value.startsWith('./styles/');
            if (!ok) {
