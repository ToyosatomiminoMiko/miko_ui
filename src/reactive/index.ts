/**
 * 库自己的响应式层:一层**薄封装**,不是重写(docs/ui-library-extraction-plan.md
 * U3 = 路线 B,U7 = vendored).
 *
 ...
 * ## 它替掉的是什么
 *
 * 每个控件过去要手写一条"值 → 控件 → 值"的双向同步回路,每条回路都是一个
 * "第二真相源".有了这一层,状态可以只有一个,控件由绑定驱动.
 *
 * ## 三条硬约束(§4.1 末;违反任何一条这条路线就退化)
 *
 ...
 *    是显式选择的批处理.库里一次都不用,"更新路径不引调度器"这条硬约束
 *    (R2)才成立,900 行手写 DOM 桩也才能继续用(桩里没有微任务队列).
 * 3. **不当状态管理库用**:只取 `signal` / `computed` / `effect`,不做跨模块
 *    store,也不用它的自动订阅糖(那需要 JSX).公开面里没有 `batch` /
 *    `untracked` / `action` / `createModel` 的转出.
 *
 *    > 计划 §4.1 的约束 3 里还列了 `onCleanup`.实测 vendored 的
 *    > `@preact/signals-core@1.14.4` **没有这个导出**:1.x 的清理语义是
 *    > `effect` 的回调**返回**一个清理函数(下一次重跑或销毁时执行).库直接
...
export interface ReadonlySignal<T> {
     ...
     *
     * 立刻回调是刻意的:订阅方通常要把当前值推给下游(如 `renderer.setRadius`),
     * 等第一次变化才推就会漏掉初值.同一个值不会重复通知(`Object.is` 短路).
     */
 ...
 *
 * **同步执行**(包括第一次):`set()` 之后订阅回调在同一个调用栈里跑完,
 * 不排队、不等帧.这是手写 DOM 桩还能用的前提(R2).
 *
 ...
 *
 * 这条联合类型是 P3 的**增量路径**:传普通值 = 今天的行为,传 signal 才订阅.
 * 于是面板可以一个控件一个控件地转过去,不必一次性推翻.
 */
 ...
 *
 * 用于"同一个状态有两套 UI 表示"的场景,而且两套表示都能被用户直接改:
 * 相机投影模式(真相是 `CamMode`,开关看到的是"勾选 = 正交")与点的大小
 * (真相是实际半径,面板在比例模式下显示的是倍数).没有它,这种控件就得在
 ...
 * 的状态对象上(如 `createViewState()` 的结果),而不是每次装配面板时新建一个.
 */
